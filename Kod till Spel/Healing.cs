...
namespace Kod_till_Spel
{
    class Healing
    {
        ...
        public void _Healing()
        {
            ...
            {
                Console.WriteLine(CenterText.CenterTexts("Din hjälte har redan fullt HP"));
            }
            else
            {
                Console.WriteLine(CenterText.CenterTexts("Din hjälte börjar Meditera för att återställa HP"));
                while (hero.hp < hero.maxHp)
                {
                    if (hero.hp < hero.maxHp * 0.25)    // När hjälten har mindre än 25% av maxHP
                    {
                        hero.hp += 1;
                        Console.WriteLine(CenterText.CenterTexts($"Nuvarande hp: {hero.hp}"));
                        Thread.Sleep(300);
                ...
                Console.WriteLine(CenterText.CenterTexts($"Din hjälte har {hero.hp}hp av {hero.hp}hp"));
                Thread.Sleep(850);
            }
        }
    }

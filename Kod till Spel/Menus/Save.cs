...
public class Save
{
    ...
    public static void SaveHeroes(Hero newHero, string filename)
    {
        ...
        if (existingHero != null)
        {
            ...
            Console.WriteLine(CenterText.CenterTexts($"Hjälten [{newHero.name}] uppdaterades."));
        }
        else
        {
            // Annars, lägg till den nya hjälten
            heroes.Add(newHero);
            Console.WriteLine(CenterText.CenterTexts($"En ny hjälte [{newHero.name}] har sparats."));
        }
